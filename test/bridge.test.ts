...
import { CursorBridge } from "../src/bridge/index.js"
import { eventsToCompletion } from "../src/http/completion-response.js"
import type { ChatCompletionRequest } from "../src/openai-types.js"
import type { AgentRequest, CursorAgent, CursorRuntime, CursorTurnInput } from "../src/cursor/index.js"
import type { CursorModelListItem } from "../src/models/index.js"
  ...
  const runtime: CursorRuntime & {
    ...
    async createAgent(input) {
      ...
      const agent: CursorAgent = {
        agentId,
        async send(sendInput: CursorTurnInput) {
...
test("text-only completion streams deltas once and reports usage", async () => {
  ...
  assert.equal(runtime.created[0]?.tools, "none")
...
test("hold-mode tool calls pause the run until tool results arrive", async () => {
  ...
  assert.equal(runtime.created[0]?.tools, "mcp")
  assert.equal(
